...
extension UIColor {
    ...
    convenience init(rgba_throws rgba: String) throws {
        ...
        guard let hexString: String = rgba.substringToIndex(rgba.startIndex.advancedBy(1)),
    ...
    convenience init?(name: String) throws {
        guard name.length != 0 else {
            throw UIColorInputError.UnableScanNameColor
        }
        let allColors = [
            "aliceblue": "#F0F8FFFF",
        ...
        if let cleanedName:String = name.stringByReplacingOccurrencesOfString(" ", withString: "").lowercaseString {
            if let hexStringColor = allColors[cleanedName] {
                try self.init(rgba_throws: hexStringColor)
                return
            } else {
                throw UIColorInputError.MissingHashMarkAsPrefix
            }
